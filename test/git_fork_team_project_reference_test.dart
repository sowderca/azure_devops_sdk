import 'package:azure_devops_sdk/api.dart';
import 'package:test/test.dart';

// tests for GitForkTeamProjectReference
void main() {
  var instance = GitForkTeamProjectReference();

  group('test GitForkTeamProjectReference', () {
    // Project abbreviation.
    // String abbreviation (default value: null)
    test('to test the property `abbreviation`', () async {
      // TODO
    });

    // Url to default team identity image.
    // String defaultTeamImageUrl (default value: null)
    test('to test the property `defaultTeamImageUrl`', () async {
      // TODO
    });

    // The project's description (if any).
    // String description (default value: null)
    test('to test the property `description`', () async {
      // TODO
    });

    // Project identifier.
    // String id (default value: null)
    test('to test the property `id`', () async {
      // TODO
    });

    // Project last update time.
    // DateTime lastUpdateTime (default value: null)
    test('to test the property `lastUpdateTime`', () async {
      // TODO
    });

    // Project name.
    // String name (default value: null)
    test('to test the property `name`', () async {
      // TODO
    });

    // Project revision.
    // int revision (default value: null)
    test('to test the property `revision`', () async {
      // TODO
    });

    // Project state.
    // String state (default value: null)
    test('to test the property `state`', () async {
      // TODO
    });

    // Url to the full version of the object.
    // String url (default value: null)
    test('to test the property `url`', () async {
      // TODO
    });

    // Project visibility.
    // String visibility (default value: null)
    test('to test the property `visibility`', () async {
      // TODO
    });


  });

}
