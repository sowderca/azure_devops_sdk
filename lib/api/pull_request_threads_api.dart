...
class PullRequestThreadsApi {
  ...
  PullRequestThreadsApi([ApiClient apiClient]) : apiClient = apiClient ?? defaultApiClient;
  ...
  Future<GitPullRequestCommentThread> callGet(String organization, String repositoryId, int pullRequestId, int threadId, String project, String apiVersion, { int $iteration, int $baseIteration }) async {
    ...
    String path = "/{organization}/{project}/_apis/git/repositories/{repositoryId}/pullRequests/{pullRequestId}/threads/{threadId}".replaceAll("{format}","json").replaceAll("{" + "organization" + "}", organization.toString()).replaceAll("{" + "repositoryId" + "}", repositoryId.toString()).replaceAll("{" + "pullRequestId" + "}", pullRequestId.toString()).replaceAll("{" + "threadId" + "}", threadId.toString()).replaceAll("{" + "project" + "}", project.toString());
    ...
    if($iteration != null) {
      queryParams.addAll(_convertParametersForCollectionFormat("", "$iteration", $iteration));
    }
    if($baseIteration != null) {
      queryParams.addAll(_convertParametersForCollectionFormat("", "$baseIteration", $baseIteration));
    }
    ...
    if(response.statusCode >= 400) {
      ...
      return apiClient.deserialize(_decodeBodyBytes(response), 'GitPullRequestCommentThread') as GitPullRequestCommentThread;
    } else {
      return null;
    }
  }
  ...
  Future<GitPullRequestCommentThread> create(String organization, String repositoryId, int pullRequestId, String project, String apiVersion, GitPullRequestCommentThread body) async {
    ...
    // create path and map variables
    String path = "/{organization}/{project}/_apis/git/repositories/{repositoryId}/pullRequests/{pullRequestId}/threads".replaceAll("{format}","json").replaceAll("{" + "organization" + "}", organization.toString()).replaceAll("{" + "repositoryId" + "}", repositoryId.toString()).replaceAll("{" + "pullRequestId" + "}", pullRequestId.toString()).replaceAll("{" + "project" + "}", project.toString());
  ...
  Future<List<GitPullRequestCommentThread>> list(String organization, String repositoryId, int pullRequestId, String project, String apiVersion, { int $iteration, int $baseIteration }) async {
    ...
    if(response.statusCode >= 400) {
      ...
      return (apiClient.deserialize(_decodeBodyBytes(response), 'List<GitPullRequestCommentThread>') as List).map((item) => item as GitPullRequestCommentThread).toList();
    } else {
      return null;
    }
  }
