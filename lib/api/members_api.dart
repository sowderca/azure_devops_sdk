...
class MembersApi {
  ...
  Future add(String organization, String groupId, String memberId, String apiVersion) async {
    ...
    String path = "/{organization}/_apis/GroupEntitlements/{groupId}/members/{memberId}".replaceAll("{format}","json").replaceAll("{" + "organization" + "}", organization.toString()).replaceAll("{" + "groupId" + "}", groupId.toString()).replaceAll("{" + "memberId" + "}", memberId.toString());
  ...
  Future<PagedGraphMemberList> callGet(String organization, String groupId, String apiVersion, { int maxResults, String pagingToken }) async {
    ...
    // create path and map variables
    String path = "/{organization}/_apis/GroupEntitlements/{groupId}/members".replaceAll("{format}","json").replaceAll("{" + "organization" + "}", organization.toString()).replaceAll("{" + "groupId" + "}", groupId.toString());
    ...
    if(pagingToken != null) {
      queryParams.addAll(_convertParametersForCollectionFormat("", "pagingToken", pagingToken));
    }
      queryParams.addAll(_convertParametersForCollectionFormat("", "api-version", apiVersion));
    ...
    if(response.statusCode >= 400) {
      ...
      return apiClient.deserialize(_decodeBodyBytes(response), 'PagedGraphMemberList') as PagedGraphMemberList;
    } else {
      return null;
    }
  }
