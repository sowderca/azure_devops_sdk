...
class BaseDeploymentInput {
  ...
  @override
  String toString() {
    return 'BaseDeploymentInput[condition=$condition, jobCancelTimeoutInMinutes=$jobCancelTimeoutInMinutes, overrideInputs=$overrideInputs, timeoutInMinutes=$timeoutInMinutes, ]';
  }

  BaseDeploymentInput.fromJson(Map<String, dynamic> json) {
    if (json == null) return;
  ...
  static List<BaseDeploymentInput> listFromJson(List<dynamic> json) {
    return json == null ? List<BaseDeploymentInput>() : json.map((value) => BaseDeploymentInput.fromJson(value)).toList();
  }

  static Map<String, BaseDeploymentInput> mapFromJson(Map<String, dynamic> json) {
    var map = Map<String, BaseDeploymentInput>();
    if (json != null && json.isNotEmpty) {
      json.forEach((String key, dynamic value) => map[key] = BaseDeploymentInput.fromJson(value));
    }
    return map;
  }
}
