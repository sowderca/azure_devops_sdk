...
class GitConflictDirectoryFile {
  ...
  @override
  String toString() {
    return 'GitConflictDirectoryFile[links=$links, conflictId=$conflictId, conflictPath=$conflictPath, conflictType=$conflictType, mergeBaseCommit=$mergeBaseCommit, mergeOrigin=$mergeOrigin, mergeSourceCommit=$mergeSourceCommit, mergeTargetCommit=$mergeTargetCommit, resolutionError=$resolutionError, resolutionStatus=$resolutionStatus, resolvedBy=$resolvedBy, resolvedDate=$resolvedDate, url=$url, ]';
  }

  GitConflictDirectoryFile.fromJson(Map<String, dynamic> json) {
    if (json == null) return;
    if (json['_links'] == null) {
      links = null;
    } else {
  ...
  static List<GitConflictDirectoryFile> listFromJson(List<dynamic> json) {
    return json == null ? List<GitConflictDirectoryFile>() : json.map((value) => GitConflictDirectoryFile.fromJson(value)).toList();
  }

  static Map<String, GitConflictDirectoryFile> mapFromJson(Map<String, dynamic> json) {
    var map = Map<String, GitConflictDirectoryFile>();
    if (json != null && json.isNotEmpty) {
      json.forEach((String key, dynamic value) => map[key] = GitConflictDirectoryFile.fromJson(value));
    }
    return map;
  }
}
