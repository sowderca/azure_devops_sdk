...
class TestPlanUpdateParams {
  ...
  @override
  String toString() {
    return 'TestPlanUpdateParams[areaPath=$areaPath, buildDefinition=$buildDefinition, buildId=$buildId, description=$description, endDate=$endDate, iteration=$iteration, name=$name, owner=$owner, releaseEnvironmentDefinition=$releaseEnvironmentDefinition, startDate=$startDate, state=$state, testOutcomeSettings=$testOutcomeSettings, ]';
  }

  TestPlanUpdateParams.fromJson(Map<String, dynamic> json) {
    if (json == null) return;
  ...
  static List<TestPlanUpdateParams> listFromJson(List<dynamic> json) {
    return json == null ? List<TestPlanUpdateParams>() : json.map((value) => TestPlanUpdateParams.fromJson(value)).toList();
  }

  static Map<String, TestPlanUpdateParams> mapFromJson(Map<String, dynamic> json) {
    var map = Map<String, TestPlanUpdateParams>();
    if (json != null && json.isNotEmpty) {
      json.forEach((String key, dynamic value) => map[key] = TestPlanUpdateParams.fromJson(value));
    }
    return map;
  }
}
