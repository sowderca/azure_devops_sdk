...
class GitConflictAddRename {
  ...
  @override
  String toString() {
    return 'GitConflictAddRename[links=$links, conflictId=$conflictId, conflictPath=$conflictPath, conflictType=$conflictType, mergeBaseCommit=$mergeBaseCommit, mergeOrigin=$mergeOrigin, mergeSourceCommit=$mergeSourceCommit, mergeTargetCommit=$mergeTargetCommit, resolutionError=$resolutionError, resolutionStatus=$resolutionStatus, resolvedBy=$resolvedBy, resolvedDate=$resolvedDate, url=$url, ]';
  }

  GitConflictAddRename.fromJson(Map<String, dynamic> json) {
    if (json == null) return;
    if (json['_links'] == null) {
      links = null;
    } else {
  ...
  static List<GitConflictAddRename> listFromJson(List<dynamic> json) {
    return json == null ? List<GitConflictAddRename>() : json.map((value) => GitConflictAddRename.fromJson(value)).toList();
  }

  static Map<String, GitConflictAddRename> mapFromJson(Map<String, dynamic> json) {
    var map = Map<String, GitConflictAddRename>();
    if (json != null && json.isNotEmpty) {
      json.forEach((String key, dynamic value) => map[key] = GitConflictAddRename.fromJson(value));
    }
    return map;
  }
}
