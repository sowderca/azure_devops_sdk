...
class GitPullRequestCommentThread {
  ...
  @override
  String toString() {
    return 'GitPullRequestCommentThread[links=$links, comments=$comments, id=$id, identities=$identities, isDeleted=$isDeleted, lastUpdatedDate=$lastUpdatedDate, properties=$properties, publishedDate=$publishedDate, status=$status, threadContext=$threadContext, ]';
  }

  GitPullRequestCommentThread.fromJson(Map<String, dynamic> json) {
    if (json == null) return;
    if (json['_links'] == null) {
      links = null;
    } else {
  ...
  static List<GitPullRequestCommentThread> listFromJson(List<dynamic> json) {
    return json == null ? List<GitPullRequestCommentThread>() : json.map((value) => GitPullRequestCommentThread.fromJson(value)).toList();
  }

  static Map<String, GitPullRequestCommentThread> mapFromJson(Map<String, dynamic> json) {
    var map = Map<String, GitPullRequestCommentThread>();
    if (json != null && json.isNotEmpty) {
      json.forEach((String key, dynamic value) => map[key] = GitPullRequestCommentThread.fromJson(value));
    }
    return map;
  }
}
