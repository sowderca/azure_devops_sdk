...
class GitConflictRename1to2 {
  ...
  @override
  String toString() {
    return 'GitConflictRename1to2[links=$links, conflictId=$conflictId, conflictPath=$conflictPath, conflictType=$conflictType, mergeBaseCommit=$mergeBaseCommit, mergeOrigin=$mergeOrigin, mergeSourceCommit=$mergeSourceCommit, mergeTargetCommit=$mergeTargetCommit, resolutionError=$resolutionError, resolutionStatus=$resolutionStatus, resolvedBy=$resolvedBy, resolvedDate=$resolvedDate, url=$url, ]';
  }

  GitConflictRename1to2.fromJson(Map<String, dynamic> json) {
    if (json == null) return;
    if (json['_links'] == null) {
      links = null;
    } else {
  ...
  static List<GitConflictRename1to2> listFromJson(List<dynamic> json) {
    return json == null ? List<GitConflictRename1to2>() : json.map((value) => GitConflictRename1to2.fromJson(value)).toList();
  }

  static Map<String, GitConflictRename1to2> mapFromJson(Map<String, dynamic> json) {
    var map = Map<String, GitConflictRename1to2>();
    if (json != null && json.isNotEmpty) {
      json.forEach((String key, dynamic value) => map[key] = GitConflictRename1to2.fromJson(value));
    }
    return map;
  }
}
