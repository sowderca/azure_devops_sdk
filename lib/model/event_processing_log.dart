...
class EventProcessingLog {
  ...
  @override
  String toString() {
    return 'EventProcessingLog[activityId=$activityId, description=$description, endTime=$endTime, errors=$errors, id=$id, logType=$logType, messages=$messages, properties=$properties, source=$source, startTime=$startTime, warnings=$warnings, ]';
  }

  EventProcessingLog.fromJson(Map<String, dynamic> json) {
    if (json == null) return;
  ...
  static List<EventProcessingLog> listFromJson(List<dynamic> json) {
    return json == null ? List<EventProcessingLog>() : json.map((value) => EventProcessingLog.fromJson(value)).toList();
  }

  static Map<String, EventProcessingLog> mapFromJson(Map<String, dynamic> json) {
    var map = Map<String, EventProcessingLog>();
    if (json != null && json.isNotEmpty) {
      json.forEach((String key, dynamic value) => map[key] = EventProcessingLog.fromJson(value));
    }
    return map;
  }
}
