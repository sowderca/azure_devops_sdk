...
class TfvcChangesetRef {
  ...
  @override
  String toString() {
    return 'TfvcChangesetRef[links=$links, author=$author, changesetId=$changesetId, checkedInBy=$checkedInBy, comment=$comment, commentTruncated=$commentTruncated, createdDate=$createdDate, url=$url, ]';
  }

  TfvcChangesetRef.fromJson(Map<String, dynamic> json) {
    if (json == null) return;
    if (json['_links'] == null) {
      links = null;
    } else {
  ...
  static List<TfvcChangesetRef> listFromJson(List<dynamic> json) {
    return json == null ? List<TfvcChangesetRef>() : json.map((value) => TfvcChangesetRef.fromJson(value)).toList();
  }

  static Map<String, TfvcChangesetRef> mapFromJson(Map<String, dynamic> json) {
    var map = Map<String, TfvcChangesetRef>();
    if (json != null && json.isNotEmpty) {
      json.forEach((String key, dynamic value) => map[key] = TfvcChangesetRef.fromJson(value));
    }
    return map;
  }
}
