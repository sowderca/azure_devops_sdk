...
class GitChange {
  ...
  @override
  String toString() {
    return 'GitChange[author=$author, displayUri=$displayUri, id=$id, location=$location, message=$message, messageTruncated=$messageTruncated, pusher=$pusher, timestamp=$timestamp, type=$type, ]';
  }

  GitChange.fromJson(Map<String, dynamic> json) {
    if (json == null) return;
    if (json['author'] == null) {
      author = null;
    } else {
  ...
  static List<GitChange> listFromJson(List<dynamic> json) {
    return json == null ? List<GitChange>() : json.map((value) => GitChange.fromJson(value)).toList();
  }

  static Map<String, GitChange> mapFromJson(Map<String, dynamic> json) {
    var map = Map<String, GitChange>();
    if (json != null && json.isNotEmpty) {
      json.forEach((String key, dynamic value) => map[key] = GitChange.fromJson(value));
    }
    return map;
  }
}
