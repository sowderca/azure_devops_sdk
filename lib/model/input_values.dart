...
class InputValues {
  ...
  @override
  String toString() {
    return 'InputValues[defaultValue=$defaultValue, error=$error, inputId=$inputId, isDisabled=$isDisabled, isLimitedToPossibleValues=$isLimitedToPossibleValues, isReadOnly=$isReadOnly, possibleValues=$possibleValues, ]';
  }

  InputValues.fromJson(Map<String, dynamic> json) {
    if (json == null) return;
  ...
  static List<InputValues> listFromJson(List<dynamic> json) {
    return json == null ? List<InputValues>() : json.map((value) => InputValues.fromJson(value)).toList();
  }

  static Map<String, InputValues> mapFromJson(Map<String, dynamic> json) {
    var map = Map<String, InputValues>();
    if (json != null && json.isNotEmpty) {
      json.forEach((String key, dynamic value) => map[key] = InputValues.fromJson(value));
    }
    return map;
  }
}
