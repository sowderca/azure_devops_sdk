...
class GitConflictEditDelete {
  ...
  @override
  String toString() {
    return 'GitConflictEditDelete[links=$links, conflictId=$conflictId, conflictPath=$conflictPath, conflictType=$conflictType, mergeBaseCommit=$mergeBaseCommit, mergeOrigin=$mergeOrigin, mergeSourceCommit=$mergeSourceCommit, mergeTargetCommit=$mergeTargetCommit, resolutionError=$resolutionError, resolutionStatus=$resolutionStatus, resolvedBy=$resolvedBy, resolvedDate=$resolvedDate, url=$url, ]';
  }

  GitConflictEditDelete.fromJson(Map<String, dynamic> json) {
    if (json == null) return;
    if (json['_links'] == null) {
      links = null;
    } else {
  ...
  static List<GitConflictEditDelete> listFromJson(List<dynamic> json) {
    return json == null ? List<GitConflictEditDelete>() : json.map((value) => GitConflictEditDelete.fromJson(value)).toList();
  }

  static Map<String, GitConflictEditDelete> mapFromJson(Map<String, dynamic> json) {
    var map = Map<String, GitConflictEditDelete>();
    if (json != null && json.isNotEmpty) {
      json.forEach((String key, dynamic value) => map[key] = GitConflictEditDelete.fromJson(value));
    }
    return map;
  }
}
