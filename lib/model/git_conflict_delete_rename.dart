...
class GitConflictDeleteRename {
  ...
  @override
  String toString() {
    return 'GitConflictDeleteRename[links=$links, conflictId=$conflictId, conflictPath=$conflictPath, conflictType=$conflictType, mergeBaseCommit=$mergeBaseCommit, mergeOrigin=$mergeOrigin, mergeSourceCommit=$mergeSourceCommit, mergeTargetCommit=$mergeTargetCommit, resolutionError=$resolutionError, resolutionStatus=$resolutionStatus, resolvedBy=$resolvedBy, resolvedDate=$resolvedDate, url=$url, ]';
  }

  GitConflictDeleteRename.fromJson(Map<String, dynamic> json) {
    if (json == null) return;
    if (json['_links'] == null) {
      links = null;
    } else {
  ...
  static List<GitConflictDeleteRename> listFromJson(List<dynamic> json) {
    return json == null ? List<GitConflictDeleteRename>() : json.map((value) => GitConflictDeleteRename.fromJson(value)).toList();
  }

  static Map<String, GitConflictDeleteRename> mapFromJson(Map<String, dynamic> json) {
    var map = Map<String, GitConflictDeleteRename>();
    if (json != null && json.isNotEmpty) {
      json.forEach((String key, dynamic value) => map[key] = GitConflictDeleteRename.fromJson(value));
    }
    return map;
  }
}
