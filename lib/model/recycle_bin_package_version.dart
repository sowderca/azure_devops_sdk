...
class RecycleBinPackageVersion {
  ...
  @override
  String toString() {
    return 'RecycleBinPackageVersion[directUpstreamSourceId=$directUpstreamSourceId, id=$id, isCachedVersion=$isCachedVersion, isDeleted=$isDeleted, isLatest=$isLatest, isListed=$isListed, normalizedVersion=$normalizedVersion, packageDescription=$packageDescription, publishDate=$publishDate, storageId=$storageId, version=$version, views=$views, ]';
  }

  RecycleBinPackageVersion.fromJson(Map<String, dynamic> json) {
    if (json == null) return;
  ...
  static List<RecycleBinPackageVersion> listFromJson(List<dynamic> json) {
    return json == null ? List<RecycleBinPackageVersion>() : json.map((value) => RecycleBinPackageVersion.fromJson(value)).toList();
  }

  static Map<String, RecycleBinPackageVersion> mapFromJson(Map<String, dynamic> json) {
    var map = Map<String, RecycleBinPackageVersion>();
    if (json != null && json.isNotEmpty) {
      json.forEach((String key, dynamic value) => map[key] = RecycleBinPackageVersion.fromJson(value));
    }
    return map;
  }
}
