...
class FeedCore {
  ...
  @override
  String toString() {
    return 'FeedCore[capabilities=$capabilities, fullyQualifiedId=$fullyQualifiedId, fullyQualifiedName=$fullyQualifiedName, id=$id, isReadOnly=$isReadOnly, name=$name, project=$project, upstreamEnabled=$upstreamEnabled, upstreamSources=$upstreamSources, view=$view, viewId=$viewId, viewName=$viewName, ]';
  }

  FeedCore.fromJson(Map<String, dynamic> json) {
    if (json == null) return;
  ...
  static List<FeedCore> listFromJson(List<dynamic> json) {
    return json == null ? List<FeedCore>() : json.map((value) => FeedCore.fromJson(value)).toList();
  }

  static Map<String, FeedCore> mapFromJson(Map<String, dynamic> json) {
    var map = Map<String, FeedCore>();
    if (json != null && json.isNotEmpty) {
      json.forEach((String key, dynamic value) => map[key] = FeedCore.fromJson(value));
    }
    return map;
  }
}
