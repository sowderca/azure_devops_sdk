...
class GitConflictEditEdit {
  ...
  @override
  String toString() {
    return 'GitConflictEditEdit[links=$links, conflictId=$conflictId, conflictPath=$conflictPath, conflictType=$conflictType, mergeBaseCommit=$mergeBaseCommit, mergeOrigin=$mergeOrigin, mergeSourceCommit=$mergeSourceCommit, mergeTargetCommit=$mergeTargetCommit, resolutionError=$resolutionError, resolutionStatus=$resolutionStatus, resolvedBy=$resolvedBy, resolvedDate=$resolvedDate, url=$url, ]';
  }

  GitConflictEditEdit.fromJson(Map<String, dynamic> json) {
    if (json == null) return;
    if (json['_links'] == null) {
      links = null;
    } else {
  ...
  static List<GitConflictEditEdit> listFromJson(List<dynamic> json) {
    return json == null ? List<GitConflictEditEdit>() : json.map((value) => GitConflictEditEdit.fromJson(value)).toList();
  }

  static Map<String, GitConflictEditEdit> mapFromJson(Map<String, dynamic> json) {
    var map = Map<String, GitConflictEditEdit>();
    if (json != null && json.isNotEmpty) {
      json.forEach((String key, dynamic value) => map[key] = GitConflictEditEdit.fromJson(value));
    }
    return map;
  }
}
