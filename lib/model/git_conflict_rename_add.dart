...
class GitConflictRenameAdd {
  ...
  @override
  String toString() {
    return 'GitConflictRenameAdd[links=$links, conflictId=$conflictId, conflictPath=$conflictPath, conflictType=$conflictType, mergeBaseCommit=$mergeBaseCommit, mergeOrigin=$mergeOrigin, mergeSourceCommit=$mergeSourceCommit, mergeTargetCommit=$mergeTargetCommit, resolutionError=$resolutionError, resolutionStatus=$resolutionStatus, resolvedBy=$resolvedBy, resolvedDate=$resolvedDate, url=$url, ]';
  }

  GitConflictRenameAdd.fromJson(Map<String, dynamic> json) {
    if (json == null) return;
    if (json['_links'] == null) {
      links = null;
    } else {
  ...
  static List<GitConflictRenameAdd> listFromJson(List<dynamic> json) {
    return json == null ? List<GitConflictRenameAdd>() : json.map((value) => GitConflictRenameAdd.fromJson(value)).toList();
  }

  static Map<String, GitConflictRenameAdd> mapFromJson(Map<String, dynamic> json) {
    var map = Map<String, GitConflictRenameAdd>();
    if (json != null && json.isNotEmpty) {
      json.forEach((String key, dynamic value) => map[key] = GitConflictRenameAdd.fromJson(value));
    }
    return map;
  }
}
