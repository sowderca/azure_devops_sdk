...
class GitConflictDeleteEdit {
  ...
  @override
  String toString() {
    return 'GitConflictDeleteEdit[links=$links, conflictId=$conflictId, conflictPath=$conflictPath, conflictType=$conflictType, mergeBaseCommit=$mergeBaseCommit, mergeOrigin=$mergeOrigin, mergeSourceCommit=$mergeSourceCommit, mergeTargetCommit=$mergeTargetCommit, resolutionError=$resolutionError, resolutionStatus=$resolutionStatus, resolvedBy=$resolvedBy, resolvedDate=$resolvedDate, url=$url, ]';
  }

  GitConflictDeleteEdit.fromJson(Map<String, dynamic> json) {
    if (json == null) return;
    if (json['_links'] == null) {
      links = null;
    } else {
  ...
  static List<GitConflictDeleteEdit> listFromJson(List<dynamic> json) {
    return json == null ? List<GitConflictDeleteEdit>() : json.map((value) => GitConflictDeleteEdit.fromJson(value)).toList();
  }

  static Map<String, GitConflictDeleteEdit> mapFromJson(Map<String, dynamic> json) {
    var map = Map<String, GitConflictDeleteEdit>();
    if (json != null && json.isNotEmpty) {
      json.forEach((String key, dynamic value) => map[key] = GitConflictDeleteEdit.fromJson(value));
    }
    return map;
  }
}
