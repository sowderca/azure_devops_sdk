...
class DashboardResponse {
  ...
  @override
  String toString() {
    return 'DashboardResponse[links=$links, description=$description, eTag=$eTag, id=$id, name=$name, ownerId=$ownerId, position=$position, refreshInterval=$refreshInterval, url=$url, widgets=$widgets, ]';
  }
  ...
  static List<DashboardResponse> listFromJson(List<dynamic> json) {
    return json == null ? List<DashboardResponse>() : json.map((value) => DashboardResponse.fromJson(value)).toList();
  }

  static Map<String, DashboardResponse> mapFromJson(Map<String, dynamic> json) {
    var map = Map<String, DashboardResponse>();
    if (json != null && json.isNotEmpty) {
      json.forEach((String key, dynamic value) => map[key] = DashboardResponse.fromJson(value));
    }
    return map;
  }
}
