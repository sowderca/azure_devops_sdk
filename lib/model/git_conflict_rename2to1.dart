...
class GitConflictRename2to1 {
  ...
  @override
  String toString() {
    return 'GitConflictRename2to1[links=$links, conflictId=$conflictId, conflictPath=$conflictPath, conflictType=$conflictType, mergeBaseCommit=$mergeBaseCommit, mergeOrigin=$mergeOrigin, mergeSourceCommit=$mergeSourceCommit, mergeTargetCommit=$mergeTargetCommit, resolutionError=$resolutionError, resolutionStatus=$resolutionStatus, resolvedBy=$resolvedBy, resolvedDate=$resolvedDate, url=$url, ]';
  }

  GitConflictRename2to1.fromJson(Map<String, dynamic> json) {
    if (json == null) return;
    if (json['_links'] == null) {
      links = null;
    } else {
  ...
  static List<GitConflictRename2to1> listFromJson(List<dynamic> json) {
    return json == null ? List<GitConflictRename2to1>() : json.map((value) => GitConflictRename2to1.fromJson(value)).toList();
  }

  static Map<String, GitConflictRename2to1> mapFromJson(Map<String, dynamic> json) {
    var map = Map<String, GitConflictRename2to1>();
    if (json != null && json.isNotEmpty) {
      json.forEach((String key, dynamic value) => map[key] = GitConflictRename2to1.fromJson(value));
    }
    return map;
  }
}
