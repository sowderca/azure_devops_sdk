...
class NotificationDeliveryLog {
  ...
  @override
  String toString() {
    return 'NotificationDeliveryLog[activityId=$activityId, description=$description, endTime=$endTime, errors=$errors, id=$id, logType=$logType, messages=$messages, properties=$properties, source=$source, startTime=$startTime, warnings=$warnings, ]';
  }

  NotificationDeliveryLog.fromJson(Map<String, dynamic> json) {
    if (json == null) return;
  ...
  static List<NotificationDeliveryLog> listFromJson(List<dynamic> json) {
    return json == null ? List<NotificationDeliveryLog>() : json.map((value) => NotificationDeliveryLog.fromJson(value)).toList();
  }

  static Map<String, NotificationDeliveryLog> mapFromJson(Map<String, dynamic> json) {
    var map = Map<String, NotificationDeliveryLog>();
    if (json != null && json.isNotEmpty) {
      json.forEach((String key, dynamic value) => map[key] = NotificationDeliveryLog.fromJson(value));
    }
    return map;
  }
}
