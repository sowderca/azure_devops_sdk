...
class MinimalPackageVersion {
  ...
  @override
  String toString() {
    return 'MinimalPackageVersion[directUpstreamSourceId=$directUpstreamSourceId, id=$id, isCachedVersion=$isCachedVersion, isDeleted=$isDeleted, isLatest=$isLatest, isListed=$isListed, normalizedVersion=$normalizedVersion, packageDescription=$packageDescription, publishDate=$publishDate, storageId=$storageId, version=$version, views=$views, ]';
  }

  MinimalPackageVersion.fromJson(Map<String, dynamic> json) {
    if (json == null) return;
  ...
  static List<MinimalPackageVersion> listFromJson(List<dynamic> json) {
    return json == null ? List<MinimalPackageVersion>() : json.map((value) => MinimalPackageVersion.fromJson(value)).toList();
  }

  static Map<String, MinimalPackageVersion> mapFromJson(Map<String, dynamic> json) {
    var map = Map<String, MinimalPackageVersion>();
    if (json != null && json.isNotEmpty) {
      json.forEach((String key, dynamic value) => map[key] = MinimalPackageVersion.fromJson(value));
    }
    return map;
  }
}
