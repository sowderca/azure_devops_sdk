...
class GitConflictRenameDelete {
  ...
  @override
  String toString() {
    return 'GitConflictRenameDelete[links=$links, conflictId=$conflictId, conflictPath=$conflictPath, conflictType=$conflictType, mergeBaseCommit=$mergeBaseCommit, mergeOrigin=$mergeOrigin, mergeSourceCommit=$mergeSourceCommit, mergeTargetCommit=$mergeTargetCommit, resolutionError=$resolutionError, resolutionStatus=$resolutionStatus, resolvedBy=$resolvedBy, resolvedDate=$resolvedDate, url=$url, ]';
  }

  GitConflictRenameDelete.fromJson(Map<String, dynamic> json) {
    if (json == null) return;
    if (json['_links'] == null) {
      links = null;
    } else {
  ...
  static List<GitConflictRenameDelete> listFromJson(List<dynamic> json) {
    return json == null ? List<GitConflictRenameDelete>() : json.map((value) => GitConflictRenameDelete.fromJson(value)).toList();
  }

  static Map<String, GitConflictRenameDelete> mapFromJson(Map<String, dynamic> json) {
    var map = Map<String, GitConflictRenameDelete>();
    if (json != null && json.isNotEmpty) {
      json.forEach((String key, dynamic value) => map[key] = GitConflictRenameDelete.fromJson(value));
    }
    return map;
  }
}
