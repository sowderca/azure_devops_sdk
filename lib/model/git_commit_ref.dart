...
class GitCommitRef {
  ...
  @override
  String toString() {
    return 'GitCommitRef[links=$links, author=$author, changeCounts=$changeCounts, changes=$changes, comment=$comment, commentTruncated=$commentTruncated, commitId=$commitId, committer=$committer, parents=$parents, push=$push, remoteUrl=$remoteUrl, statuses=$statuses, url=$url, workItems=$workItems, ]';
  }

  GitCommitRef.fromJson(Map<String, dynamic> json) {
    if (json == null) return;
    if (json['_links'] == null) {
      links = null;
    } else {
  ...
  static List<GitCommitRef> listFromJson(List<dynamic> json) {
    return json == null ? List<GitCommitRef>() : json.map((value) => GitCommitRef.fromJson(value)).toList();
  }

  static Map<String, GitCommitRef> mapFromJson(Map<String, dynamic> json) {
    var map = Map<String, GitCommitRef>();
    if (json != null && json.isNotEmpty) {
      json.forEach((String key, dynamic value) => map[key] = GitCommitRef.fromJson(value));
    }
    return map;
  }
}
