...
class TfvcChange {
  ...
  @override
  String toString() {
    return 'TfvcChange[author=$author, displayUri=$displayUri, id=$id, location=$location, message=$message, messageTruncated=$messageTruncated, pusher=$pusher, timestamp=$timestamp, type=$type, ]';
  }

  TfvcChange.fromJson(Map<String, dynamic> json) {
    if (json == null) return;
    if (json['author'] == null) {
      author = null;
    } else {
  ...
  static List<TfvcChange> listFromJson(List<dynamic> json) {
    return json == null ? List<TfvcChange>() : json.map((value) => TfvcChange.fromJson(value)).toList();
  }

  static Map<String, TfvcChange> mapFromJson(Map<String, dynamic> json) {
    var map = Map<String, TfvcChange>();
    if (json != null && json.isNotEmpty) {
      json.forEach((String key, dynamic value) => map[key] = TfvcChange.fromJson(value));
    }
    return map;
  }
}
